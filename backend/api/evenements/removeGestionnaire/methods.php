<?php
    ...
    function removeGestionnaireInEvent($token, $body, $IdEvent){
        ...
        if(!(in_array($IdCible, $arrayOfGestionnaire))){
            throw new Exception ("Ce gestionnaire ne gère pas cet événement !");
        }
        ...
        if(mysqli_num_rows($result) > 0){
            if($row = mysqli_fetch_array($result)){
                ...
                $query = "DELETE FROM Gerer WHERE IdEvenement=" . $IdEvenement . " AND IdGestionnaire = ". $IdGestionnaire . ";";
                mysqli_query($conn, $query);
                echo json_encode(array("sucess"=> true));
                http_response_code(200);
            } else {
                throw new Exception ("Aucun gestionnaire avec cette id !");
            }
        } else {
            throw new Exception ("Aucun gestionnaire avec cette id !");
        }
