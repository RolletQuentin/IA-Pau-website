<?php
// Exemple utilisation : http://localhost/api/questionnaires/createReponseQuestionnaire/
// Headers requis
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!empty($donnees->IdQuestionnaire) && !empty($donnees->IdEquipe) && !empty($donnees->DateReponse)) {
        // Ici on a reçu les données
        // On hydrate notre objet
        $questionnaire->IdQuestionnaire = $donnees->IdQuestionnaire;

        if($questionnaire->createReponseQuestionnaireByIdEquipe($donnees->IdEquipe, $donnees->DateReponse)) {
            // Ici la création à fonctionné
            // On envoi un code 201 (ajout)
            http_response_code(201);
            echo json_encode(["message" => "Reponse questionnaire bien crée"]);
        } else{
