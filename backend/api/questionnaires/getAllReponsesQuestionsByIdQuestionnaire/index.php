<?php
// Exemple utilisation : http://localhost/api/questionnaires/getAllReponsesQuestionsByIdQuestionnaire/?id=2
// Headers requis
...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    if(!empty($IdQuestionnaire)){
        ...
        $stmt = $questionnaire->getAllReponsesQuestionsByIdQuestionnaire();

        // On vérifie si on a au moins 1 produit
        if($stmt->rowCount() > 0){
            // On initialise un tableau associatif
            $tableauQuestions = [];
            $tableauQuestions['Reponse questions'] = [];
            ...
            while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
                ...
                $prod = [
                    ...
                    "IdEquipe" => $IdQuestionnaire,
                    "Reponse" => $Question
                ];

                $tableauQuestions['Reponse questions'][] = $prod;
            }
