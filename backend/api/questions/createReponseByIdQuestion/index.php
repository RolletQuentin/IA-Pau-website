<?php
// Exemple utilisation : http://localhost/api/questions/createReponseByIdQuestion/
// Headers requis
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    if(!empty($donnees->IdQuestion) && !empty($donnees->IdEquipe) && !empty($donnees->Reponse)) {
        // Ici on a reçu les données
        // On hydrate notre objet
        $question->IdQuestion = $donnees->IdQuestion;

        if($question->createReponseByIdQuestion($donnees->IdEquipe, $donnees->Reponse)) {
            // Ici la création à fonctionné
            // On envoi un code 201 (ajout)
            http_response_code(201);
            echo json_encode(["message" => "Réponse enregistrée"]);
        } else{
